...
> **Якщо потрібно простими словами**, без коду й формул — починаємо з [`ablation_explained.md`](./ablation_explained.md): аналогія з контрольними у школі, прив'язана до наших 18 сценаріїв.
...
- Усереднюємо ці Δ по всіх парах `(scenario_id, seed)`, і отримуємо середній ефект компонента, у якому *вплив складності сценарію взаємно скорочується* при відніманні (бо однаковий у зменшуваному й від'ємнику).
...
> Ці кроки відповідають коду в [`scripts/analyze_contributions.py`](../../scripts/analyze_contributions.py). Кожну секцію нижче можна розгорнути за потреби.
...
**Чому це чесніше за `mean(B) − mean(A)`?** У цьому прикладі обидва числа збігаються, бо вибірка збалансована. Але уявімо, що `A` був прогнаний лише на легких сценаріях (середнє ≈ 0.70), а `B` — лише на складних (середнє ≈ 0.55). Наївне `mean(B) − mean(A) = −0.15` сказало б «B погіршує», тоді як насправді ми порівнювали різні задачі. Pairing усуває цей confound: кожна Δ міряна на **тому самому сценарії під тим самим seed-ом**, і його внутрішня складність взаємно скорочується при відніманні — лишається лише ефект компонента.
...
**Чому ж тоді p-value лишається у `contributions.csv`?** Wilcoxon-тест відповідає на іншу інференційну задачу, ніж bootstrap-CI: він перевіряє, наскільки спостережений Δ-вектор сумісний із нульовою гіпотезою про відсутність ефекту, тоді як інтервал оцінює діапазон, у якому з імовірністю 95 % лежить справжнє середнє. Дві процедури спираються на різні припущення — rank-based-тест проти resampling-у — тож їхня згода фактично служить перехресною валідацією: систематична розбіжність між CI і p-value є діагностичним сигналом перевірити Δ-вектор на структурні артефакти (виродженість вибірки, дублікати, систематичні NaN). Обчислення тесту — один виклик `scipy.stats.wilcoxon(...)` (~20 µs на Δ-вектор), тож збереження поля має нульову інженерну ціну.

P-value **не** керує вердиктом дашборду через відому консервативність signed-rank-тесту при малих *n*: на типовому розмірі вибірки `n = 18` (`SEEDS = 1`) тест має низьку статистичну потужність і регулярно лишає реальні ефекти з `p > 0.05` попри узгоджений напрям Δ (Conover, *Practical Nonparametric Statistics*, 3-тє вид., §5.7). Bootstrap percentile CI на тих самих даних дає інтервал, ширина якого еквівалентна заявленій непевності, тож рішення «компонент стабільно покращує метрику» приймається на CI, а p-value лишається як супутній diagnostic у CSV.
...
> **Як підвищити статистичну потужність:** збільшити `SEEDS`. При `SEEDS = 3` (типовий research-режим) `n = 54`, CI стає ~1.7× вужчим (за √3), і слабкі компоненти, які при `SEEDS = 1` губляться у шумі, стають видимими. Платимо за це токенами і часом — батч стає у 3 рази довшим і дорожчим.
...
Потрібен швидкий smoke-test? `make ablation SEEDS=1 SCENARIO=N01 CONFIG=E` запустить **одну** клітинку — зручно для перевірки wiring без спалення квоти.
