...
- `RetryingLLMProvider` повторює спробу тільки якщо внутрішній рівень кинув `LLMError`. Timeout рахується як `LLMError`, тому ретраїться (хоча часто марно — модель знову стільки ж).
- `SanitizingLLMProvider` може кинути `LLMError` на порожньому вивбіді — це єдиний випадок, коли retry отримує шанс виправити «модель видала лише CoT».
