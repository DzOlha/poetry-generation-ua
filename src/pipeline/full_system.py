...
def check_poem(
    poem_text: str, meter: str, foot_count: int, rhyme_scheme: str, stress_dict: StressDict
) -> PipelineReport:
    ...
    rhyme_accuracy = (
        (sum(1 for p in rhyme_results.pairs if p.rhyme_ok) / len(rhyme_results.pairs)) if rhyme_results.pairs else 1.0
    )
...
def run_full_pipeline(
    ...
    for it in range(max_iterations):
        ...
        report = check_poem(
            poem, meter=meter, foot_count=foot_count, rhyme_scheme=rhyme_scheme, stress_dict=stress_dict
        )
    else:
        report = PipelineReport(
            meter_ok=report.meter_ok,
