...
class RagPromptBuilder(IPromptBuilder):
    ...
    def build(
        ...
        return (
            ...
            "DO NOT think out loud. Do NOT produce any reasoning, drafts, scansion, "
            "analysis, preamble, postscript, or self-corrections. If you need to "
            "reason about meter or rhyme, do it silently and output only the final "
            "poem. The entire reply must be exactly "
            f"{request.structure.total_lines} Ukrainian poem lines — nothing before, "
            "nothing after, no blank separators other than one newline between lines.\n"
            "STRICT FORMAT RULES — violating any is a failure:\n"
            "- The reply MUST start with a Cyrillic letter (first word of line 1). "
