...
Якщо вірш у корпусі вже має передобчислений вектор (`embedding: [float, ...]`), retriever використовує його напряму **без повторного кодування**. Це прискорює роботу при великих корпусах. Поточний `uk_poetry_corpus.json` ембедінгів не містить — вони обчислюються on-the-fly при кожному пошуку.
...
2. Для кожного вірша в корпусі:
   а) беремо poem.embedding (якщо є), або encode(poem.text)  →  poem_vec
   б) cosine_similarity(theme_vec, poem_vec)
        ↓
...
```python
def build_rag_prompt(theme, meter, rhyme_scheme, retrieved, stanzas=4) -> str:
    ...
    return (
        ...
        f"Generate a {stanzas}-stanza Ukrainian poem."
    )
```
...
Meter: ямб
Rhyme scheme: ABAB
Generate a 4-stanza Ukrainian poem.
```
...
[2] retriever.retrieve(theme)   → top-5 семантично близьких víршів
...
[8] check_poem(новий вірш) → повторити кроки 6-8
...
18 кюрованих сценаріїв у трьох категоріях:

### NORMAL (N01–N05) — типові запити

| ID | Тема | Метр | Рима | Навіщо |
|----|------|------|------|--------|
| N01 | Весна в лісі | ямб 4 ст. | ABAB | Найпоширеніша форма |
| N02 | Кохання | хорей 4 ст. | AABB | Народна пісенна традиція |
| N03 | Батьківщина | ямб 5 ст. | ABBA | Шевченківський стиль |
| N04 | Самотність | амфібрахій 3 ст. | ABAB | Менш поширений метр |
| N05 | Місто вночі | дактиль 3 ст. | AABB | Урбаністична тематика |
...
КОРПУС (corpus/uk_poetry_corpus.json)
  ...
  └─────────────┘
        │
        └── (повтор валідації, max_iterations разів)
                    │
                    ▼
...
*Документ згенеровано автоматично на основі вихідного коду проекту.*
*Актуальний для версії: `src/pipeline/full_system.py`, `src/evaluation/`, `src/meter/`, `src/rhyme/`, `src/retrieval/`.*
